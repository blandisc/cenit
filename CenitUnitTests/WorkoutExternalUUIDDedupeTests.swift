...
///
/// The key moved from `noop:strength:<id>` to `cenit:strength:<id>`. That rename is only safe
...
final class WorkoutExternalUUIDDedupeTests: XCTestCase {
    ...
    func testLegacyKeyKeepsTheNoopPrefix() {
        XCTAssertEqual(WorkoutMirrorKey.legacyExternalUUID(for: sessionId), "noop:strength:\(sessionId)")
    }
    ...
    func testDedupeAcceptsBothPrefixes() {
        ...
        XCTAssertEqual(keys, ["cenit:strength:\(sessionId)", "noop:strength:\(sessionId)"])

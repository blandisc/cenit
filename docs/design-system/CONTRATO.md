...
| no-unique-keys-dictionary (`Dictionary(uniqueKeysWithValues:)` en Screens; puro, FER-502) | árbol Screens | idem | idem |
...
```json
{
  ...
  "rules": {
    ...
    "no-unique-keys-dictionary": {"pre-commit": "tree:Cenit/Screens", "verify-quick": "tree:Cenit/Screens", "design-lint": "tree:Cenit/Screens"}
  }
}
```
<!-- gate-matrix:end -->

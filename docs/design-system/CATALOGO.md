...
| Pastilla de estado Liquid | `LiquidStatePill` | `LiquidGlass/LiquidStatePill.swift` | Estado vivo/listo sobre cristal (`.pastillaSolida`) o chip de valencia Δ% — sustituye `statusPill` a mano y chips de signo. | No procedencia (`LiquidOrigenChip`/`LiquidOrigenBadge`); no filtro removible (`LiquidChipSeleccion`). |

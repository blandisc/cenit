...
| Selector de periodo (Liquid) | `LiquidRangeSelector` | `LiquidGlass/LiquidRangeSelector.swift` | Selector de periodo en Cuerpo / hojas Liquid (S·M·3M·…) con tick del tono. | No reinventar el periodo. `SegmentedPillControl` no entra en una pantalla nueva. |
| Control segmentado (Instrumento) | `SegmentedPillControl` | `Components.swift` | Solo donde ya está, para una opción que no es un periodo. No para pantallas nuevas. | No como selector de periodo (usa `LiquidRangeSelector`). No adoptarlo en una pantalla nueva. |
...
| Barra de tabs | `LiquidTabBar` | `LiquidGlass/LiquidTabBar.swift` | Galería y previews. El dock de la app es la tab bar del sistema. | No montarla en el shell ni en pantallas nuevas; no para un segmentado de contenido. |
...
| Stepper de ajuste Entrenar | `EntrenarStepper` | `Entrenar/EntrenarStepper.swift` | Control − valor + de hoja-herramienta (incremento, reloj de descanso) — tallas `.fila`/`.hoja`. | No reinventar −/+ a mano; el caller formatea el valor. |
| Fila secundaria de contexto (Entrenar) | `EntrenarFilaCarga` | `Entrenar/EntrenarFilaCarga.swift` | La fila compacta y NEUTRA bajo el hilo del veredicto en la portada de Entrenar: punto `verdeCarga` (solo con banda) + rótulo en versalitas («CONTEXTO · CARGA», `LiquidType.regla`) + la palabra de banda en tinta, o «Calibrando»; «›» solo con acción y abre la Hoja de carga. Papel pelón: sin vidrio, sin borde, sin pastilla (FER-488; decisión 2026-09-15 §2/§6). | Nunca para el veredicto (eso es `EntrenarHilo`); nunca con un número, ratio o escala (por eso `TrainingLoadStrip` no sirve aquí); nunca con el color de bandera (`Flag.color()`, `verdePrimario`, `atencion`, `negativo`): la carga no vota. No montarla sin carga calculable ni esfuerzo registrado — se oculta; nunca «Calibrando» sobre orbe dormido. |
| Toggle Liquid | `LiquidToggleStyle / .liquid` | `LiquidGlass/LiquidToggleStyle.swift` | Switch cromo El Eje (tinta900/tinta10/papelTarjeta) en hojas Liquid — `.toggleStyle(.liquid)`. | No teñir el track con el color del dato. El estilo `.instrumento` no es para pantallas nuevas. |
| Deslizador Liquid | `LiquidSlider` | `LiquidGlass/LiquidSlider.swift` | Ajuste continuo o por pasos sobre un rango en hojas Liquid (margen, reserva, caída, bpm) — pista de vidrio + relleno del `base` del tono + perilla de papel; construido sobre `liquidScrubPan`, así que convive con el ScrollView de la hoja (no roba el scroll vertical). | No dejar `Slider` crudo con `.tint` en pantallas Liquid; no para incremento discreto −/+ (usa `EntrenarStepper`); no para un riel de solo lectura (usa `LiquidBarraProgreso`). |
| Título de flujo Liquid | `LiquidFlowTitle` | `LiquidGlass/LiquidFlowTitle.swift` | Cabecera kicker + displayS de pantallas empujadas sin salida propia (Tickets). | No cuando ya hay `EntrenarHojaCabecera`. `InstrumentoFlowTitle` no es para pantallas nuevas. |
...
| Bloque patrón (Instrumento) | `.patternBlock(_:bar:)` | `SessionInstruments.swift` | Solo donde ya está. No para pantallas nuevas. | En una pantalla nueva usa `LiquidPatternBlock`. No pintar `theme.patternBlock` a mano. |
| Bloque patrón (Liquid) | `LiquidPatternBlock` | `LiquidGlass/LiquidPatternBlock.swift` | «Tu patrón» / lectura quieta en hojas Liquid — overline + líneas + barra del tono, sin vidrio. | No envolverlo en `liquidGlass`. `.patternBlock` no es para pantallas nuevas. |
| Gráfica de tendencia (Liquid) | `LiquidTrendChart` | `LiquidGlass/LiquidTrendChart.swift` | Serie temporal en una pantalla de El Eje. Es la gráfica de tendencia para una pantalla nueva. | No para una línea inline diminuta (usa `Sparkline`). `TrendChart` no entra en una pantalla nueva. |
| Gráfica de tendencia (compartida) | `TrendChart` | `TrendChart.swift` | Solo donde ya está. No para pantallas nuevas. | En una pantalla nueva usa `LiquidTrendChart`. No para una línea inline diminuta (usa `Sparkline`). |
| Sparkline inline | `Sparkline` | `Sparkline.swift` | Tendencia diminuta dentro de un tile (Hoy / live-HR). | No como gráfica principal de una pantalla de detalle (usa `LiquidTrendChart`). |
...
| Aviso Liquid | `LiquidAviso` | `LiquidGlass/LiquidAviso.swift` | Heads-up / desconexión / nudge / aviso con pie de acciones en pantalla Liquid — `LiquidPatternBlock` + `liquidTarjetaSeccion` (receta HealthAlertBanner); icono/CTA/pie opcionales. | No snack de deshacer (`UndoToast`); no error de escritura (`.saveErrorToast`). |
| Tarjeta de una vez | `LiquidUnaVezTipStyle` | `LiquidGlass/LiquidUnaVez.swift` | Hito del motor o novedad mayor de una versión (FER-428 · D4/D2): `TipView` de TipKit vestido como un módulo más de la columna — `liquidGlass(tono:regimen:)` del huésped (sobrio en Hoy/Tendencias, mosaico en Entrenar), kicker `regla` en `tono.rotulo`, cuerpo, puerta de texto en `verdeProfundo` (la primera `Tips.Action`) y «Entendido» gris a la derecha. Se aplica en cada sitio de anclaje con `.tipViewStyle(...)`. | No para consejos anclados a un control (`LiquidConsejoTipStyle`, el estilo de raíz); no aviso/alerta (`LiquidAviso`); nunca modal ni encima del héroe/la palabra; sin ícono, ilustración ni confeti. |

...
F3 colapsa la ramificación por fuente (`usesBanda`/`usesAppleHealth`, pineada a Apple) y F4 borra el motor
...
el `guard mode.usesBanda` (línea 209) lo haga inalcanzable: no se pueden borrar los motores hasta que muera
el orquestador, y el orquestador es el único no-op que `usesBanda` sigue protegiendo.
...
`AppModel*`, y de todo `IntelligenceEngine` **sobrevive UN solo símbolo vivo: `bandaOnlyHistory`** (usado por
`AppModel+Illness.swift:85`). Todo lo demás muere.

## Supuestos

- F1 (borrado de `ProtocoloBanda`) y F2 (pantallas reloj/viaje) están DONE y verdes. Verificado:
...
- Greenfield Apple-only: `SourceModeStore.mode` está pineado a `.appleHealthOnly` y rechaza cualquier cambio
  (`SourceModeStore.swift:15-17`). Por tanto `usesBanda == false` y `usesAppleHealth == true` son **constantes**.
...
- El esquema greenfield v1, el naming `banda→apple` y el borrado de tablas de streams crudos son **F7**.
...
| `let estimatesSteps` (:34) | **MUERE** | Solo `AppModel:161` pasa `false`; su test D3 (`testEstimatesStepsIsExactlyTheBandaBit`) vivía en ProtocoloBanda tests → **ya borrado en F1** |
...
| `analyzeRecent(maxDays:force:)` (:186) | **MUERE** (no-op: `guard mode.usesBanda else { return [] }` :209) | `AppModel+Analysis:60,66,139`, `AppModel+Maintenance:37` — todas descartan o reciben `[]` |
...
| **`static bandaOnlyHistory(_:appleHealthDays:)` (:840)** | **VIVE** | **`AppModel+Illness.swift:85`** (prod) + `IllnessWatchSourceTests`, `IntelligenceBaselinePriorTests` (tests) |

**Conclusión: 1 símbolo vivo (`bandaOnlyHistory`), todo lo demás muere con la clase.**

---

## 2. Lo VIVO: dónde re-ubicar `bandaOnlyHistory`
...
public static func bandaOnlyHistory(_ hist: [DailyMetric], appleHealthDays: Set<String>) -> [DailyMetric] {
...
**Frontera con F6 (CRÍTICA):** el brief y el plan maestro dicen «`bandaOnlyHistory`→identidad» en F4. **Eso es
un error de granularidad y se DIFIERE a F6.** Razón: en `AppModel+Illness` (`:85-91`) `bandaDays` solo se
consume en la rama `signalSource == .band` (`: bandaDays`, línea 91); en Apple-only `signalSource` es
...
`.band` es **read-model = F6**. Volver `bandaOnlyHistory` identidad AHORA (antes del gate `/cso`+`/estadistico`)
...
6. `Cenit/App/AppModel+Analysis.swift`:
   - `startAnalysis()`: tras el `guard self.sources.mode.usesBanda else { return }` (:53) todo el cuerpo restante (:54-69: los dos `intelligence.analyzeRecent()`, el `Task.sleep` de 6 s, el `while` de 15 min) es MUERTO. Colapsar: `startAnalysis` termina tras la secuencia de launch-refresh (:42-47) → `return`. Quitar el `guard usesBanda` y todo lo que le sigue.
   - `resumeForegroundAnalysis()` (:95): el `guard sources.mode.usesBanda else { <force refresh>; return }` siempre toma el `else`; el cuerpo del `else` (:97-101) PASA A SER el cuerpo del método. Borrar la cola `.band` (:103-104, `startAnalysis()`).
...
9. `Cenit/Data/Repository.swift` — colapsar cada sitio (la rama pineada gana):
   ...
   - `:637,:674` `guard usesBanda else { return [] }` → `return []` directo (métodos band muertos; conservan firma, devuelven vacío).
   - `:730-731` `imported/computed = []`; `:732` `apple = <read apple-health>`.
   - `:786` `guard usesBanda else { return nil }` → `return nil`.
   - `:1206-1207` `useBanda = false`, `useApple = true` (simplificar el respectingMode).
   - `:1351,:1365` `usesAppleHealth` → constante `true`.
10. `Cenit/Screens/TodayView.swift`:
    - `:401-419` `whyEmptyExplanation`: `usesBanda` es `false` → conservar SOLO las cadenas de la rama Apple ya autorizadas (`:408,:414,:417`); borrar las variantes band. **No es copy nuevo** (las cadenas Apple ya existen), es poda de la rama inalcanzable.
    - `:548` `dayMap: usesBanda ? stressDayMap : nil` → `nil`.
    - `:1055` `if usesBanda { PullIndicator(...) }` → borrar el bloque band (dial 24h / BPM en vivo son de la banda).
11. `Cenit/Screens/CuerpoView.swift:465` `dayMap: usesBanda ? stressDayMap : nil` → `nil`.
12. `Cenit/Screens/AjustesView.swift:267`: `if usesBanda { <navRow CyclePhase> }` es MUERTO → borrar el bloque (la fila Cycle phase). `CyclePhaseView` queda sin punto de entrada; **NO borrar la pantalla aquí** (es consumidor F6/MED). Anotar el huérfano.
...
- **Riesgo top 1 — cruzar a F6 sin querer (SDNN↔RMSSD).** El llamador vivo más fácil de romper es
  `AppModel+Illness.applyIllnessEvaluation` (`:85-91`): mezcla `bandaOnlyHistory` (F4) con
  `SourceLens.maskForBaseline(keep:.apple)` + `signalSource` (F6). **Regla dura para esta fase: solo reubicar
  `bandaOnlyHistory` verbatim y re-apuntar el símbolo. NO tocar `signalSource`, `SourceLens.Source`,
  `keep:.apple/.band`, ni volver `bandaOnlyHistory` identidad.** Eso es F6, con gate `/cso`+`/estadistico`+Grok.
...
- **Riesgo 3 — la fuente computada `(sufijo de fuente computada)` deja de existir.** Tras borrar el engine nadie escribe
  `banda-computada` (daily/sleep/workouts/metricSeries). Consumidores que la leen (`Repository.computedSeries`,
  `latestCircadianPhase`, el tile `steps_est`) degradan a vacío — no crashean. Verificar en el compile iOS que
  ningún path espera filas `(sufijo de fuente computada)` no-vacías. Las tablas siguen DORMIDAS hasta F7.
...
- F6: `SourceLens`/`SourceFusion`/`keep:.band`, la semántica SDNN↔RMSSD, la rama `.band` de AppModel+Illness,
  el borrado final de `bandaOnlyHistory`, y los 7 consumidores del read-model.
- F7: esquema greenfield v1, naming `banda→apple`, `DashboardSnapshot.includeBandaSeries`, el enum
...
-   desde los streams crudos de la banda (HRV-dominante). Corre off-main, gateada por `usesBanda`.

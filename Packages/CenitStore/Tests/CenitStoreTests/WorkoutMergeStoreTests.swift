...
/// imported (`fuente-importada`), Apple Health (`apple-health`), and the on-device detector (`fuente-derivada`) —
...
final class WorkoutMergeStoreTests: XCTestCase {
    ...
    private let detected = "fuente-derivada"   // la fuente derivada es la importada + un sufijo
    ...
    private func union(_ store: CenitStore, from: Int, to: Int) async throws -> [WorkoutRow] {
        var rows = try await store.workouts(deviceId: imported, from: from, to: to, limit: 5000)
    ...
    func testThreeSourceBucketsCoexistAndUnionInOneWindow() async throws {
        ...
        try await store.upsertWorkouts([wk(1_000, 40, sport: "Run", source: "importado")], deviceId: imported)
        ...
        let importedRows = try await store.workouts(deviceId: imported, from: 0, to: 100_000, limit: 100)
        ...
        XCTAssertEqual(importedRows.map(\.sport), ["Run"])
    ...
    func testIdenticalStartTsAcrossSourcesAreDistinctRows() async throws {
        ...
        let store = try await CenitStore.inMemory()
        try await store.upsertWorkouts([wk(5_000, 30, sport: "Run", source: "importado")], deviceId: imported)
    ...
    func testDeleteDetectedBucketLeavesStrapAndAppleUntouched() async throws {
        ...
        let store = try await CenitStore.inMemory()
        try await store.upsertWorkouts([wk(8_000, 20, sport: "Run", source: "importado")], deviceId: imported)
    ...
    func testWindowExcludesWorkoutsOutsideRange() async throws {
        ...
        let store = try await CenitStore.inMemory()
        try await store.upsertWorkouts([wk(1_000, 10, sport: "Old", source: "importado"),
                                        wk(50_000, 10, sport: "Recent", source: "importado")], deviceId: imported)
        let windowed = try await store.workouts(deviceId: imported, from: 40_000, to: 60_000, limit: 100)

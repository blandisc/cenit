...
public enum SourceFusion {
    ...
    /// Pure wrapper over the CenitAnalytics engine, kept pure so the refresh can call it
    ...
    /// the live-strain path uses, so the estimate never jumps between sources (FER-883 /cso). nil
    ...
    /// CenitAnalytics never imports CenitTraining.
    ...
    /// How far back a strength session may SYNTHESIZE a day row when no source has any history
    ...
    /// (`dailyMetric.strain` stays whatever the measured source wrote). Without this a session logged without a
    ...
    ///   • "steps"           — Apple's pedometer count (`appleAgg.steps`) vs the on-device figure
    ...
    ///                         enter).
    ///   • "active_kcal"     — Apple's aggregate (`appleAgg.activeKcal`) vs each on-device row's HR-only
    ...
    public static func mergeSleepSessions(imported: [CachedSleepSession], computed: [CachedSleepSession],
        ...
        let onDevice = Array(byStart.values)
        func overlapsOnDevice(_ a: CachedSleepSession) -> Bool {
            onDevice.contains { $0.startTs <= a.endTs && $0.endTs >= a.startTs }
        }
        let merged = onDevice + apple.filter { !overlapsOnDevice($0) }
    ...
    public static func appleSleepsNotCoveredOnDevice(apple: [CachedSleepSession],
        ...
        apple.filter { a in !onDevice.contains { $0.startTs <= a.endTs && $0.endTs >= a.startTs } }
...
private extension DailyMetric {
    ...
    /// Health row for the same day). Fields the on-device row already carries always win — only the gaps Apple can fill
    /// change. recovery/strain stay on-device-only in practice because Apple rows carry them as nil. This is

...
// value inside the row. Lives HERE and not in CenitAnalytics because it reads `SetEntry`/`SetKind`
// (ARCHITECTURE §3: CenitAnalytics never imports CenitTraining).
public enum SessionRPE {

    /// Lowest / highest rating the session-effort row offers (RIR-anchored, Zourdos 2016:
    /// 10 = 0 reps in reserve). Mirrors `SessionRPELoad.rpeMin/rpeMax` in CenitAnalytics, which

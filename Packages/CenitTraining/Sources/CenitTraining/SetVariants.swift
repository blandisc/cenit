...
//
// Viven aquí, en CenitTraining (puro, Foundation-only), y no en la pantalla, porque son REGLAS DEL
// ENTRENAMIENTO, no de una vista: si mañana el escalón del drop deja de ser −20 %, cambia UNA vez.
// Devuelven kilos CRUDOS a propósito: CenitTraining no puede importar `PlateMath` (vive en
// CenitAnalytics, que no depende de CenitTraining — ver docs/ARCHITECTURE.md), así que el redondeo a

...
// CenitAnalytics consume them directly. Ids are UUID strings (user-authored,
...
/// the rest of CenitAnalytics already uses — so there is at most one routine per day (`weekday` is the
...
public struct StrengthSessionSnapshot: Codable, Sendable, Equatable {
    ...
    public struct RunSnapshot: Codable, Sendable, Equatable {
        ...
        /// A Codable mirror of `CenitAnalytics.ProgressionState` (FER-189) — `CenitTraining` can't
        /// import `CenitAnalytics` (the dependency only runs the other way, per the package graph),

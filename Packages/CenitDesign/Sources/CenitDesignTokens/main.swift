...
public let catalogEntries: [CatalogEntry] = [
    ...
    CatalogEntry(rol: "Barra de tabs", simbolo: "LiquidTabBar",
                 ...
                 cuandoUsarlo: "Galería y previews. El dock de la app es la tab bar del sistema.",
                 cuandoNo: "No montarla en el shell ni en pantallas nuevas; no para un segmentado de contenido."),
    ...
    CatalogEntry(rol: "Campo de búsqueda Liquid", simbolo: "LiquidCampoBusqueda",
                 ...
                 cuandoNo: "No reinventar HStack+TextField a mano; no para campos de métrica teñidos (usa `LiquidCampoMetrica`); no campo de texto genérico (usa `LiquidCampoTexto`)."),
    CatalogEntry(rol: "Campo de texto Liquid", simbolo: "LiquidCampoTexto",
                 archivo: "LiquidGlass/LiquidCampoTexto.swift",
                 cuandoUsarlo: "Campo de texto El Eje (nombre, deporte, notas, números) — padding + `.superficieSolida` + `cuerpoLista`; teclado/multilínea/sufijo opcionales (FER-339).",
                 cuandoNo: "No para búsqueda con lupa (usa `LiquidCampoBusqueda`); no campo teñido de métrica (usa `LiquidCampoMetrica`); no dejar `TextField` suelto en pantallas Liquid."),
    ...
    CatalogEntry(rol: "Deslizador Liquid", simbolo: "LiquidSlider",
                 ...
                 cuandoNo: "No dejar `Slider` crudo con `.tint` en pantallas Liquid; no para incremento discreto −/+ (usa `EntrenarStepper`); no para un riel de solo lectura (usa `LiquidBarraProgreso`)."),

...
public extension InstrumentoTheme {
    ...
    // MARK: Tinte por opacidad — helpers de la escala `CenitOpacity` (auditoría jul-2026, H4)
    //
    // Azúcar sobre `CenitOpacity` para el patrón más común: modular un color de dato/estado a
    // opacidad de tinte. Preferir estos en el call site (`theme.tint(theme.warning)`) sobre el
    // literal (`theme.warning.opacity(0.12)`).

    /// Fondo tintado de chip/badge (`CenitOpacity.tintFill`, 0.10).
    func tint(_ c: Color) -> Color       { c.opacity(CenitOpacity.tintFill) }
    /// Tinte enfatizado (`CenitOpacity.tintFillStrong`, 0.14).
    func tintStrong(_ c: Color) -> Color { c.opacity(CenitOpacity.tintFillStrong) }
    /// Borde tintado suave (`CenitOpacity.strokeSoft`, 0.30).
    func softStroke(_ c: Color) -> Color { c.opacity(CenitOpacity.strokeSoft) }

...
public enum LiquidMotion {
    ...
    // MARK: Puente valor-neutral desde CenitMotion (FER-280·2e)
    //
    // Alta mecánica: mismos valores exactos que los presets de `CenitMotion` aún calientes
    // en `Cenit/**`. Cero rediseño de tempo — solo un dialecto (`LiquidMotion`) y nombres
    // es-MX por rol. `gentle` (Cenit) NO se llama `gentle` aquí: `LiquidMotion.gentle` ya
    // es la duración 420 ms; el spring de casa para cambios de valor/estado es `suave`.
    //
    // Mapa viejo → nuevo (valores congelados):
    //   CenitMotion.fade         easeInOut(0.30)                         → fundido
    //   CenitMotion.gentle       spring(response: 0.5, damping: 0.8)      → suave
    //   CenitMotion.interactive  interactiveSpring(0.28, 0.82, blend 0.1) → toque
    //   CenitMotion.hero         spring(response: 0.85, damping: 0.85)    → heroe
    //   CenitMotion.countUp      easeOut(0.75)                           → conteo
    //   CenitMotion.gated(_:_)   helper Reduce Motion                    → condicionado(_:_:)

    /// 300 ms — fundido estándar de tarjetas / filtros / overlays.
    /// Origen: `CenitMotion.durationStandard` / `CenitMotion.fade` (Motion.swift).
    public static let fundidoDuration: Double = 0.30

    /// 750 ms — conteo 0→valor del recibo (una sola vez al guardar).
    /// Origen: `CenitMotion.countUp` (Motion.swift).
    public static let conteoDuration: Double = 0.75

    /// `fundido` — ease-in-out 300 ms. Origen: `CenitMotion.fade`.
    ...
    /// Origen: `CenitMotion.gentle` = spring(response: 0.5, dampingFraction: 0.8).
    ...
    /// Origen: `CenitMotion.interactive` =
    ...
    /// Origen: `CenitMotion.hero` = spring(response: 0.85, dampingFraction: 0.85).
    ...
    /// Origen: `CenitMotion.gated(_:_)` (ReduceMotion.swift).

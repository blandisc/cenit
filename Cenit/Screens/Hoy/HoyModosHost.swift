...
private struct AvisoDesconexion: View {
    ...
    private var respiro: some View {
        TimelineView(.animation(minimumInterval: LiquidMotion.intervaloSello, paused: quieto)) { tl in
            ...
            // Halo vía LiquidGlow (contorno + fill): el blur vive en la pieza, no ad-hoc.
            LiquidGlow(
                contorno: RoundedRectangle(cornerRadius: LiquidRadius.control, style: .continuous),
                stroke: LiquidColor.rojoClaro.opacity(0.35 + 0.20 * fase),  // token-exempt(optico): glow que respira
                blur: 6 + 3 * fase,

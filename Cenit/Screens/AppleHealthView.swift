...
struct AppleHealthView: View {
    ...
    /// de fingir un «sin datos» sobre una serie que sí tiene algo que decir. La altura fija es de la
    /// gráfica y de la lectura suelta, NO del vacío (FER-433): el vacío que enseña crece con el texto
    /// y a tamaños de accesibilidad no se recorta.
    @ViewBuilder
    ...
    /// El «cómo se llena» de una sección sin lecturas. La gráfica con datos lo recibe en
    /// `estadoVacio:` — parámetro obligatorio que con ≥2 puntos nunca se pinta.
    fileprivate static var emptySectionHowItFills: String {
...
/// Una serie sin ninguna lectura. FER-433: en vez del pozo mudo («sin lecturas»), el vacío que
/// enseña — qué va en esta sección y qué falta para que se llene.
private struct NoReadingsCard: View {
    var body: some View {
        LiquidVacio(
            queEs: Text(String(localized: "vacio.salud.seccion.queEs",
                               defaultValue: "This section's readings go here.")),
            comoSeLlena: Text(AppleHealthView.emptySectionHowItFills))
    }
}
